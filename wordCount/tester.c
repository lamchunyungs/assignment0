...
{
  // Test for data1.txt
  char* argv[] = {"mywc","testcases/data1.txt",NULL};
  int argc = sizeof(argv)/sizeof(char*) -1;
  count(argc,argv);
...
{
  // Test for data1.txt
  char* argv[] = {"mywc","testcases/data2.txt",NULL};
  int argc = sizeof(argv)/sizeof(char*) -1;
  count(argc,argv);
...
int main ( void )
{
  ...
  printf("\n\n##########################################\n");
  printf("# Cases Passed: [%d]/[%d]%19s\n",success,total,"#");
  printf("# Score for Assignment 1 WC: [%03d]/[%d] #\n",success * PERMARK,total * PERMARK);
  printf("##########################################\n");
	 CU_cleanup_registry();
